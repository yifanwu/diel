...
import { ExpressionValue, DerivedRelation, Command, CrossFilterChartIr, CrossFilterIr, DielAst, DielDataType, UdfType, BuiltInUdfTypes, OriginalRelation, RelationConstraints, RelationType, DielTemplate, ForeignKey, ProgramsParserIr, InsertionClause, DropClause, Column, RelationReference, RelationSelection, CompositeSelectionUnit, ColumnSelection, SetOperator, SelectionUnit, JoinAst, OrderByAst, JoinType, RawValues, AstType, Order, GroupByAst, createEmptyDielAst, ColumnConstraints, DeleteClause, ExprAst, ExprValAst, ExprFunAst, FunctionType, BuiltInFunc, ExprColumnAst, ExprType, ExprParen, ExprRelationAst, DropType, UpdateClause, Relation, NewRelationConstraints, ExprStarAst, RelationReferenceType, RelationReferenceDirect, RelationReferenceSubquery } from "./dielAstTypes";
  ...
  visitProgramBody(ctx: parser.ProgramBodyContext): Command[] {
    ...
    for (let i = 0; i < rawPrograms.length; i++) {
      ...
      if (iQuery) {
        ...
        programs.push(this.visit(uQuery) as UpdateClause);
      } else {
